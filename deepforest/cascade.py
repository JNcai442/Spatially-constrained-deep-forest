...
from . import _io, _utils,spatial_random_forest
...
def _build_regressor_predictor(
    ...
    if predictor_name == "no_spatial":
        ...
        sp_n_estimators_num=sp_n_estimators[0]
        neighbors_num=neighbors[0]
        predictor=spatial_random_forest.SpatialRandomForest(n_estimators=sp_n_estimators_num,neighbors=neighbors_num)
    return predictor
...
class BaseCascadeForest(BaseEstimator, metaclass=ABCMeta):
    def __init__(
        ...
        self.sp_n_estimators=sp_n_estimators,
    ...
    # flake8: noqa: E501
    def fit(self, X, y,g_coords_train,sample_weight=None):
        X, y = check_X_y(
            X,
            y,
            multi_output=True
        ...
        if self.use_predictor:
            ...
            if self.predictor in ("no_spatial", "spatial"):
                if is_classifier(self):
            ...
            tic = time.time()
            self.predictor_.fit(X_middle_train_, y,g_coords_train)
            toc = time.time()
        ...
        return self
    def fit_new(self, X, y,sample_weight=None):
        X, y = check_X_y(
            X,
            y,
            multi_output=True
...
class CascadeForestRegressor(BaseCascadeForest, RegressorMixin):
    ...
    def fit(self, X, y,g_coords_train=None,sample_weight=None):
            X, y = check_X_y(
                X,
                y,
                multi_output=True
            ...
            super().fit(X, y,g_coords_train,sample_weight)
    ...
    def fit_new(self, X, y,sample_weight=None):
        ...
        super().fit_new(X, y,sample_weight)
